...
export function Skills({ skills }: { skills: Skill[] }) {
  ...
  return (
    <motion.section id="skills" ref={ref}
      ...
      style={{ background: "var(--section-bg)", padding: "96px 0" }}
    >
      <div style={{ maxWidth: "1140px", margin: "0 auto", padding: "0 40px" }}>
        <div style={{ marginBottom: "56px" }}>
          <p style={{ fontSize: "11px", fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", color: "var(--blue)", marginBottom: "12px" }}>Technical Skills</p>
          <h2 style={{ fontSize: "clamp(2rem, 4vw, 3rem)", fontWeight: 900, color: "var(--heading)", lineHeight: 1.1, marginBottom: "16px" }}>What I Work With</h2>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={active}
            ...
            style={{ display: "grid", gap: "16px" }} id="skills-grid"
          >
            {shown.map((skill, i) => {
              ...
              return (
                <motion.div key={skill.id}
                  ...
                  style={{ background: "var(--card-bg)", border: "1px solid var(--card-border)", borderRadius: "14px", padding: "20px 22px", boxShadow: "0 1px 6px var(--card-shadow)", transition: "transform 0.15s, box-shadow 0.15s" }}
                  onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform = "translateY(-2px)"; (e.currentTarget as HTMLElement).style.boxShadow = "0 6px 20px var(--card-shadow)"; }}
                  onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform = "translateY(0)"; (e.currentTarget as HTMLElement).style.boxShadow = "0 1px 6px var(--card-shadow)"; }}
                >
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "12px" }}>
                    <h3 style={{ fontSize: "14px", fontWeight: 600, color: "var(--heading)" }}>{skill.name}</h3>
                    <span style={{ fontSize: "12px", fontWeight: 700, padding: "3px 8px", borderRadius: "8px", background: `${c}12`, color: c }}>{skill.proficiency}%</span>
                  </div>
                  <div style={{ height: "5px", borderRadius: "9999px", background: "var(--bg-soft)", overflow: "hidden", marginBottom: "12px" }}
                    role="progressbar" aria-valuenow={skill.proficiency} aria-valuemin={0} aria-valuemax={100} aria-label={`${skill.name} proficiency`}>
                  ...
                  <span style={{ fontSize: "11px", fontWeight: 600, padding: "4px 10px", borderRadius: "9999px", background: `${c}10`, color: c }}>{skill.category}</span>
                </motion.div>
              );
            })}
          </motion.div>
